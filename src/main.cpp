...
#if defined(NDEBUG)
# error "Dadcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x91fdf24f1664e75aa31bf4e50dd7afcd25e16898e4451f33c3faef79d95a0741");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dadcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dadcoin Signed Message:\n";
...
{
    // Dadcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Dadcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Dadcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Dadcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dadcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dadcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd525a90b9e949da2d2e1f0285f2dcc38f4b63c164650878f731a669a0e530e6c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "future of dadcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0443d43898531dae0699387b501e38e55a8803f9db78878717dd79cafdb271b11e0d91954e5ab879ce82d289a34a1d290f064feb975ae3a0bfffc7212423a5359f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1545348754;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085672889;

        if (fTestNet)
        {
            block.nTime    = 1545348742;
            block.nNonce   = 386819793;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xaf5f0e553d53416554951bd098ab52b8cb06e250efbc0422f8c09cb2e7f80613"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Dadcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DadcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DadcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DadcoinMiner(CWallet *pwallet)
{
    printf("DadcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dadcoin-miner");
    ...
    try { loop {
        ...
        printf("Running DadcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DadcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DadcoinMiner, pwallet));
}
